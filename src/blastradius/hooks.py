...
def capture(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    body = (
        ...
        + "\n\nRead them and call record_dependencies to index them.\n"
          ...
          "\n"
          "For Python, record what pyproject.toml or a requirements file "
          "declares — `[project.dependencies]` and Poetry's "
          "`[tool.poetry.dependencies]` are both dependency lists — as type "
          "python_package. Do not read poetry.lock or uv.lock; those are "
          "parsed for you.\n"
          "\n"
          "Pass each identifier exactly as written too. A Terraform submodule "
          "keeps its `//path` suffix: "
          "`terraform-aws-modules/vpc/aws//modules/vpc-endpoints` is a "
          "different module from `terraform-aws-modules/vpc/aws`, and "
          "shortening it merges two blast radii into one.\n"
          "\n"
